...
fetch(`/api/posts/category?category=${category?category:keyword}&&page=${page || 1 }`)
.then(res => res.json())
.then(data => {
    const {result, totalPages, currentPage} = data

   totalResult.textContent = result.length == 0 ? 'No results found' : ''
...
pglink.href=`/search.html?search=${category?category:keyword}&&page=${i}`
if (i == currentPage) {
